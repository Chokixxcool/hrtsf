...
TGraphAsymmErrors *getSFGraph(TString object, TString algo, TString wp, std::vector<TString> ptrange, int ialgo, int nalgos, int color);
...
void makeMatchingEffPlots(TString obj_) {
  TH1::SetDefaultSumw2(kTRUE);
  ...
  h_pt_dr0p6_r->GetYaxis()->SetRangeUser(0.,1.5);
  h_pt_dr0p6_r->GetYaxis()->SetTitle("Recontruction efficiency");
  if (obj_== "top") { h_pt_dr0p6_r->GetXaxis()->SetTitle("t quark p_{T} [GeV]"); } 
  else  { h_pt_dr0p6_r->GetXaxis()->SetTitle("W boson p_{T} [GeV]"); } 
  h_pt_dr0p6_r->GetYaxis()->SetTitleOffset(1.08);
  h_pt_dr0p6_r->GetXaxis()->SetTitleOffset(1.1);
  h_pt_dr0p6_r->Draw("P");
  h_pt_dr0p6_q_r->Draw("P sames");
